...
class LogUserRepository
{
    ...
    {
        $stmt = db()->prepare(
            'INSERT INTO log_users (action, actor_id, target_id, changes, created_at)
    ...
    {
        return (int) db()->query('SELECT COUNT(*) FROM log_users')->fetchColumn();
    }
    ...
    {
        $stmt = db()->prepare('SELECT * FROM log_users ORDER BY id DESC LIMIT :limit OFFSET :offset');
